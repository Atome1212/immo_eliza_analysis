...
def plot_generic(ax, df, column, title, xlabel, ylabel, color, categories=None, remove_unknown=True):
    ...
    df_grouped.index = [format_column_name(name) for name in df_grouped.index]  # Format the column names
    df_grouped.sort_index().plot(kind='bar', color=color, edgecolor='black', ax=ax)
...
def generate_grouped_plots(df):
    ...
    plot_generic(axs[0], df_sales, 'ConstructionYearCategory', 'Median Price by Year of Construction (Sales Only)', 'Year of Construction', 'Median Price', '#2e8b57')
